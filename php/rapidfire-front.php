...
if ( !class_exists( 'RapidFireFront' ) ) {
    class RapidFireFront extends RapidFireModel
    {
        ...
        public function __construct()
        {
            ...
            // Add Shortcodes
            add_shortcode( 'rapidfire', array( &$this, 'show_rapidfire_handler' ) );
        ...
        public function load_resources()
        {
            ...
            wp_enqueue_style( 'rapidfire_front_css', plugins_url( '/css/front.css', $mainPluginFile ) );
        }
                    ...
                    if ( $status && $status != self::NOT_PUBLISHED ) {
                        ...
                        if ( $this->get_admin_option( 'save_scores' ) == '1' ) {
                            ...
                            $out .= '
                                    ...
                                    var button' . $quiz->id . ' = $("#rapidFire' . $quiz->id . ' .buttonWrapper a");';
                            ...
                            $out .= '
                                    ...
                                    $("#rapidFire' . $quiz->id . ' .buttonWrapper").before(
                            ...
                            $out .='
                                    var nameLabel = $("#rapidFire' . $quiz->id . ' .nameLabel input");
                                    var emailLabel = $("#rapidFire' . $quiz->id . ' .emailLabel input");

                                    // when starting quiz, hide name field
                                    $("#rapidFire' . $quiz->id . ' .button.startQuiz").on("click", function(e) {
                                        var start = $(this);
                                    ...
                                    $("#rapidFire' . $quiz->id . ' .button.checkAnswer").last().on("click", function() {
                                    ...
                                    function submitScore' . $quiz->id . '() {
                                        var json = {
                                            ...
                                            score: $("#rapidFire' . $quiz->id . ' .correctResponse").length + " / " + $("#rapidFire' . $quiz->id . ' .question").length,
                                            quiz_id: ' . $quiz->id . '
                                        };
                            ...
                            if ( !$name ) {
                                $out .= '
                                        ...
                                        $("#rapidFire' . $quiz->id . '").on("change keyup", ".nameLabel input, .emailLabel input", function() {
                        ...
                        if ( $this->get_admin_option( 'share_links' ) == '1' ) {
                            $out .= '
                                    // watch final check answer button and wait for result calculation before submitting
                                    $("#rapidFire' . $quiz->id . ' .button.nextQuestion").last().on("click", function() {
                                    ...
                                    function addShareButtons' . $quiz->id . '() {
                                        var shareDiv = $("#rapidFire' . $quiz->id . ' .quizShare");

                                        if (shareDiv.length > 0) {
                                            shareDiv.empty(); // in case Try Again is enabled, reset buttons / messages

                                            var sharePrep = "<style type=\'text/css\'>"
                                                + ".rapidFireWrapper .quizShare .facebook { background: url(' . plugins_url( '/images/facebook.png' , dirname( __FILE__ ) ) . ') left top no-repeat; } "
                                                + ".rapidFireWrapper .quizShare .twitter { background: url(' . plugins_url( '/images/twitter.png' , dirname( __FILE__ ) ) . ') left top no-repeat; } "
                                                + ".rapidFireWrapper .quizShare .email { background: url(' . plugins_url( '/images/email.png' , dirname( __FILE__ ) ) . ') left top no-repeat; } "
                                                + "</style>"
                                            shareDiv.append($(sharePrep));

                                            var name  = $("#rapidFire' . $quiz->id . ' .quizName").text();
                                            var desc  = $("#rapidFire' . $quiz->id . ' .quizDescription").text();
                                            var score = $("#rapidFire' . $quiz->id . ' .quizScore span").text();
                                            var rank  = $("#rapidFire' . $quiz->id . ' .quizLevel span").text();
                                            ...
                                            $("#rapidFire' . $quiz->id . ' .twitterButton").on("click", function(e) {
                                                e.preventDefault();
                                                rapidFireTwitter(copy);
                                            });
                                            ...
                                            $("#rapidFire' . $quiz->id . ' .facebookButton").on("click", function(e) {
                                                e.preventDefault();
                                                rapidFireFacebook(copy, encodeURIComponent(desc));
                                            });
                                    ...
                                    function rapidFireTwitter(copy) {
                                    ...
                                    function rapidFireFacebook(copy, desc) {
                                        var facebookUrl = "https://www.facebook.com/dialog/feed?"
                                            ...
                                            + "name=" + copy + "&"
                                            + "description=" + desc + "&"
                                            + "redirect_uri=http://rapidfire.com/close.html";
        ...
        // WP shortcode [rapidfire]
        public function show_rapidfire_handler( $atts )
        {
            ...
            $out = $this->show_rapidfire( $id );

            return $out;
        }

        protected function show_rapidfire( $id )
        {
            global $quiz, $status, $pageQuizzes;

            $quizModel = new RapidFireModel;
            ...
            if ( $quiz ) {
                ...
                if ( $status == self::NOT_PUBLISHED ) {
                    ...
                    $out .= '
                        <div class="rapidFireWrapper" id="rapidFire' . $quiz->id . '">
                    ...
                    if ( has_action( 'rapidfire_after_result' ) ) {
                        $out .= apply_filters( 'rapidfire_after_result', $this );
                    }
