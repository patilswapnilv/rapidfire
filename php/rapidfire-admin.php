...
if ( !class_exists( 'RapidFireAdmin' ) ) {
    class RapidFireAdmin extends RapidFireModel
    {
        ...
        function generate_quiz_row( $quiz )
        {
            ...
            if ( current_user_can( 'publish_pages' ) ) {
                ...
                $actions .= '<a class="edit_quiz" title="Edit Quiz" '
                         . 'href="' . admin_url( 'admin.php?page=rapidfire-edit&id=' ) . $id . '">'
                         . '<img id="editquiz-' . $id . '" '
            ...
            $actions .= '<a class="preview_quiz preview" title="Preview Quiz" '
                     . 'href="' . admin_url( 'admin.php?page=rapidfire-preview&id=' ) . $id . '&readOnly">'
                     . '<img id="previewquiz-' . $id . '" '
                     . 'src="' . plugins_url( '/images/view.png' , dirname( __FILE__ ) ) . '"'
                     . ' width="16" height="16" alt="Preview Quiz" /></a> &nbsp; ';

            $scoreLink = '<a href="' . admin_url( 'admin.php?page=rapidfire-scores&id=' . $quiz->id ) . '">'
...
if ( class_exists( 'RapidFireAdmin' ) ) {
    global $rapidFireAdmin;
    $rapidFireAdmin = new RapidFireAdmin();
}

?>

<div class="wrap rapidFire quizList">
    <h2>RapidFire Management <?php if ( current_user_can( 'publish_pages' ) ) { // Editor or Admin Only  ?><a href="<?php echo admin_url( 'admin.php?page=rapidfire-new' ); ?>" class="add-new-h2" title="Create a new Quiz">Add New Quiz</a><?php } ?></h2>

    <?php $rapidFireAdmin->show_alert_messages(); ?>
    ...
    <table id="record_view" class="wp-list-table widefat quizzes">
        <thead>
         ...
         <tbody>
            <?php $rapidFireAdmin->list_all_quizzes(); ?>
        </tbody>
    </table>
    <div class="tablenav bottom">
        <div class="tablenav-pages one-page"><span class="displaying-num"><?php $rapidFireAdmin->get_quiz_count() ?> <?php if ( $rapidFireAdmin->get_quiz_count(true) == 1 ) { ?>quiz<?php } else { ?>quizzes<?php } ?></span>
    </div>
</div>
